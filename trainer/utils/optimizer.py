...
def cnn_param_grid_search(p):
	# unpack the params
	gt_indicator, video_stats, diff_thres, diff_mask, cnn_confidences, cnn_confidences_range, \
	...
	for thres in thresholds:
		...
		acc = v.accuracy(gt_indicator, noscope_indicator)
	...
	positive_count = sum(map(lambda x: x == 1, gt_indicator))
	negative_count = sum(map(lambda x: x == 0, gt_indicator))
	total_passthrough = (positive_passthrough * positive_count + negative_passthrough * negative_count) / (
	positive_count + negative_count)
	_total_passthrough = (positive_passthrough * params['num_true_positives'] + negative_passthrough * params[
		'num_true_negatives']) / params['num_windows']
	assert (total_passthrough == _total_passthrough)

	params['false_negative'] = fn[lower_idx]
	params['false_positive'] = fp[upper_idx]
	params['accuracy'] = 1 - (params['num_true_positives'] * fn[lower_idx] + params['num_true_negatives'] * fp[
		upper_idx]) / params['num_windows']
...
def param_search(gt_frames, noscope_stats, noscope_frames,
	...
	for skip_distance in [1]:
		...
		assert (len(windowed_diff_confidences) == len(gt_indicator))  # are the inputs the same size?
		combined = np.transpose(np.vstack((windowed_diff_confidences, gt_indicator)))
		combined_sorted = combined[combined[:, 0].argsort()] # sort by gt confidence, so we get 0,0,0,..,1,1,1,...

		cumulative_false_negatives = np.cumsum(combined_sorted, axis=0)[:, 1]
		threshold = MAX_DIFF_FALSE_NEGATIVE_RATE * np.sum(gt_indicator)
...
def main(object_name,
         gt_csv_filename, noscope_csv_filename,
         target_fn, target_fp,
         TRAIN_START_IDX, TRAIN_END_IDX):
	TARGET_CNN_FALSE_NEGATIVE_RATE = target_fn / 2.0 # cnn false negative + diff false negative = target fn
	TARGET_CNN_FALSE_POSITIVE_RATE = target_fp / 2.0
	MAX_DIFF_FALSE_NEGATIVE_RATE = target_fn / 2.0

	with open(gt_csv_filename, 'r') as gt:
		with open(noscope_csv_filename, 'r') as noscope:
			gt_frames = v.parse_gt_csv(gt, object_name, TRAIN_START_IDX, TRAIN_END_IDX)
			noscope_frames, noscope_stats = v.parse_noscope_csv(noscope, object_name, 0,
			                                                    TRAIN_END_IDX - TRAIN_START_IDX)

	# print len(gt_frames), len(noscope_frames)
	assert (len(gt_frames) == len(noscope_frames))  # were the same number of frames read?

	runtimes = param_search(gt_frames, noscope_stats, noscope_frames,
	                        TRAIN_START_IDX, TRAIN_END_IDX,
	                        TARGET_CNN_FALSE_NEGATIVE_RATE, TARGET_CNN_FALSE_POSITIVE_RATE,
	                        MAX_DIFF_FALSE_NEGATIVE_RATE,
...
if __name__ == "__main__":
	...
	gt_csv_filename = sys.argv[2]
	noscope_csv_filename = sys.argv[3]
	target_fn = float(sys.argv[4]) / 2.0  # be conservative
	target_fp = float(sys.argv[5]) / 2.0

	optimal_params = main(object_name, gt_csv_filename, noscope_csv_filename, target_fn, target_fp)
